({
    ...
    flatten: function(){
        return this.reduce(arguments, (results, value) => {
            ...
            return this.flatten.apply(this, results.concat(this.rest(value)));
        }, []);
    },
    ...
    bind: function(func, context) {
        ...
        return this.callback(function(){ return func.apply(this, this.extend(args, arguments)); });
    },
    delay: function(func, wait) {
        func = this.auraCallback(func); var args = this.rest(arguments, 2);
        return setTimeout(() => { return func.apply(this, args); }, wait);
    },
    defer: function(func) {
        this.delay(func, 1);
    },
    ...
    compose: function() {
        ...
        return this.callback(function(){
            var result = this.callback(args[0]).apply(this, arguments);
    ...
    defaults: function(obj, defaults) {
        if(this.isObject(defaults)){
            this.each(defaults, (value, key) => { 
                if(this.isUndefined(obj[key])) obj[key] = value; 
            });
        }
        return obj;
    },
    ...
    remove: function(obj, item){
        if(!this.isArray(obj) || this.isEmpty(obj)) return obj;
        if(this.isString(item)) obj.splice(this.findIndex(obj, (value) => { return value === item; }), 1);
        if(this.isNumber(item)) obj.splice(item, 1);
        if(this.isObject(item)) obj = this.remove(obj, this.findIndex(obj, this.matcher(item)));
        return obj;
    },
    indexBy: function(obj, key){
        if(!this.isArray(obj) || !this.isString(key)) return {};
        return this.reduce(obj, (result, item) => { 
            if(this.isObject(item) && this.hasOwnProperty(item, key)) {
                var value = item[key]; if(this.isString(value)) result[value] = item;
            }
            return result;
        }, {});
    },
    groupBy: function(obj, key){
        return this.reduce(obj, (result, value) => {
            var _key = value[key];
            if (this.has(result, _key)) result[_key].push(value); else result[_key] = [value];
            return result;
        }, {});
    },
    ...
    wrapPromise: function(promise){
        ...
        promise.then = this.callback(function(){ 
            var nextPromise = promise.originalThen.apply(promise.original, this.map(arguments, this.auraCallback, this));
