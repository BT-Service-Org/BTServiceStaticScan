...
import fetchLookupData from "@salesforce/apex/PlaybookLookupController.fetchLookupData";
...
export default class PlaybookLookupComponent extends LightningElement {
    ...
    @api statusConditionAdded = false;
    ...
    // NOTE I have commented this part
    ...
    fetchSearchResults() {
        fetchLookupData({ searchKey: this.searchKey, sObjectApiName: this.sObjectApiName,recordType : this.recordType , statusCondition : this.statusConditionAdded})
            .then((res) => {
                this.isSearchLoading = false;
                if (res) {
                    this.hasRecords = res.length == 0 ? false : true;
                    this.lstResult = JSON.parse(JSON.stringify(res));
                }
            })
            .catch((error) => {
                console.log("(error---> " + JSON.stringify(error));
    ...
    lookupUpdatehandler(value) {
        const oEvent = new CustomEvent("lookupupdate", {
            detail: { selectedRecord: value, indexNumber: this.indexNumber }
        });
